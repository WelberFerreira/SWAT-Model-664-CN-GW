...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      !!bb = .2 * r2 !! Precisa comentar para ativar o .exe CN
      bb = cn_lambda(j) * r2 !! Changed by welber
      pb = precipday - bb

      if (pb > 0.) then
        !!surfq(j) = pb * pb / (precipday + .8 * r2)
        surfqimp = pb * pb / (precipday + (1-cn_lambda(j)) * r2)!! Changed by welber
      end if
      ...
      if (iurban(j) > 0) then
        ...
        pb = precipday - bb
        if (pb > 0.) then
          !!surfq(j) = pb * pb / (precipday + .8 * r2) !! Precisa comentar para ativar .exe CN
          surfqimp = pb * pb / (precipday + (1-cn_lambda(j)) * r2)!! Changed by welber
        end if
