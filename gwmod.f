...
!!    revap is subtracted and rchrg is delayed (johnson, 1977)
      ...
      integer :: j
      real :: rchrg1, rchrg_karst, gw_delay, pi
      real :: deltamin, deltamax, seno, cday

      !! gw_delay    | it was created to receive the delay time
      !! deltamin    | it was created to receive the mininum delta time
      !! deltamax    | it was created to receive the maxinum delta  time
      !! seno        | it was created to receive the seno in GW Seasonal
      !! cday        | it was created to receive the current day
      !! readgw.f
      !! We get the values and reset them to zero 
      !! gw_delay_min(ihru) = gw_spyld(ihru)
      !! gw_delay_max(ihru) = shallst_n(ihru)
      !! gw_d_max_day(ihru) = gwminp(ihru)
...
!!################################################################
!! New variables, inserted by welber
!! Delay is seasonal
!!################################################################
      pi = 4.D0*ATAN(1.D0)

      !! These values come from readgw.f

      deltamin = (gw_delay_max(j) - gw_delay_min(j))/2
      deltamax = (gw_delay_max(j) + gw_delay_min(j))/2
      cday = real(iida) !! iida is the current day
      seno = Sin(((cday - gw_d_max_day(j))/365 * 2 * pi) + pi/2)
      
      gw_delay = (deltamin * seno) + deltamax
      gw_delaye(j) = Exp(-1./(gw_delay + 1.e-6))
...
!!################################################################
      ...
      if (rchrg(j) < 1.e-6) rchrg(j) = 0.!!           FLAG
      gwq_ru(j) = 0.
