...
!!    Inserted by Welber, values concerned about new CN   
      cn_alpha(ihru) = lat_orgn(ihru)
      lat_orgn(ihru) = 0.

      cn_beta(ihru) = lat_orgp(ihru)
      lat_orgp(ihru) = 0.

!!    We have put this staments in order to get the GW delay values
!!    Inserted by Welber
      gw_delay_min(ihru) = gw_spyld(ihru)
      gw_delay_max(ihru) = shallst_n(ihru)
      gw_d_max_day(ihru) = gwminp(ihru)
      gw_spyld(ihru)  = 0.0030
      shallst_n(ihru) = 0.
      gwminp(ihru)    = 0.
